...
4. Make a =.vasprc= file, and place it in your main user directory (=~/=).
   ...
   #+END_SRC
   Notes:
   1. At this time, I haven't used the =vdw_kernel.bindat= data file. We can
      download it from the VASP user portal if we need it, and we can then
      point the =vdw_kernel.bindat= variable to this location.
