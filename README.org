#+TITLE: ASE Wrapper for VASP on Kodiak
#+date: May 25, 2020
...
1. Clone this repo somewhere, using
   #+BEGIN_SOURCE sh
   git clone https://github.com/enriquepacis/vasp-ase-kodiak.git
   #+END_SOURCE
   Note:
   1. This will produce a folder =vasp= (I believe), with another =vasp=
      directory nested inside.
2. /Optional/ You might want to rename the outer =vasp=
   to something else, such as =kodiak-vasp=. I find doing so makes things
   less confusing. To do so, you can use
   #+BEGIN_SRC sh
   mv vasp kodiak-vasp
   #+END_SRC
   in the terminal from within the folder containing the outer =vasp=.
   Follow it up with =ls -lt= to verify your operation succeeded.
3. Add the =kodiak-vasp= directory (or your outer =vasp= directory) to your
   PYTHONPATH and PATH variables by adding lines like this to your
   =~/.bashrc= file
   #+BEGIN_SRC sh
   export PYTHONPATH=path-to/kodiak-vasp:$PYTHONPATH
   export PATH=path-to/kodiak-vasp/bin:$PATH

   export VASP_PP_PATH=path-to/your/psuedopotentials
   #+END_SRC
   These place-holder =path-to/your/psuedopotentials= should contain your
   POTCARs. For example, in my setup, I have the pseudopotentials in
   =~/materials/vasp/pseudo=.
...
'''
n, bands, p = calc.get_bandstructure(kpts_path=[('$L$', [0.5, 0.5, 0.5]),
                                                (r'$\Gamma$', [0.0, 0.0, 0.0]),
                                                (r'$\Gamma$', [0.0, 0.0, 0.0]),
                                                ('$X$', [0.0, 0.5, 0.5]),
                                                ('$X$', [0.0, 0.5, 0.5]),
...
if p is None:
    ...
    p.savefig(figname)
    print(f'\n#+comment Band structure for diamond Si crystal.\n[[./{figname}]]\n')
    ...
    sp.Popen(sumo_cmdlist)

    figname = os.path.join(imgdir, 'band.pdf')
    figname2 = os.path.join(imgdir, 'Si-bands-DFT.pdf')
    if os.path.exists(figname):
       os.rename(figname, figname2)
...
band structure plot:
 /ion/home/blaire/materials/vasp/jtc-vasp-py3/images/Si-bands-DFT.pdf
#+end_example
